...
impl BotConnection {
    pub(super) async fn start(
        id: i32,
        server: super::ServerPtrWeak,
        mpsc_channels: MpscChannels,
        handler: scenario::HandlerPtrWeak,
    ) {
        log::info!("create_local_connection");
        let (tx_to_local_stream, mut rx_to_local_stream) = mpsc::unbounded_channel::<Message>();
        let (tx_from_local_stream, mut rx_from_local_stream) = mpsc::unbounded_channel::<Message>();

        // TODO: send to reconnect
        if let Some(handler) = handler.upgrade() {
            handler.write().unwrap().set_id(id.to_string());
            #[cfg(not(any(feature = "flutter", feature = "cli")))]
            {
                *CUR_BOT.lock().unwrap() = Some(handler.write().unwrap().inner());
            }
            handler
                .read()
                .unwrap()
                .reconnect(rx_to_local_stream, tx_from_local_stream);
        }
        ...
        let (tx_from_cm_holder, mut rx_from_cm) = mpsc_channels.from_cm;
        ...
        let (tx_to_cm, rx_to_cm) = mpsc_channels.to_cm;
        let (tx, mut rx) = mpsc_channels.tx_rx;
        let (tx_video, mut rx_video) = mpsc_channels.tx_rx_video;
        let (tx_input, rx_input) = mpsc_channels.tx_rx_input;
        ...
        let mut conn = Self {
            ...
            tx_to_local_stream: tx_to_local_stream,
            server,
            hash,
            ...
            processing_request: Default::default(),
        ...
        tokio::spawn(async move {
            let tx_from_cm_clone = tx_from_cm.clone();
            if let Err(err) = start_ipc_to_cm(rx_to_cm, tx_from_cm, "bcm").await {
                tx_from_cm_clone.send(ipc::Data::Close);
                log::error!("ipc to bot connection manager exit: {}", err);
            }
        });
        ...
        loop {
            tokio::select! {
                ...
                msg_in = rx_from_local_stream.recv() => {
                    if let Some(msg_in) = msg_in {
                        let msg = msg_in.clone();
                        match msg.union {
                            Some(message::Union::TestDelay(msg)) => {

                            }
                            _ => {
                                log::info!("rx_from_local_stream {:#?}", msg);
                            }
                        }
                ...
                Some((instant, value)) = rx_video.recv() => {
                    ...
                    if let Err(SendError(err)) = conn.tx_to_local_stream.send((&value as &Message).clone()) {
                        conn.on_close(&err.to_string(), false).await;
                        break;
                    }
                },
                Some((instant, value)) = rx.recv() => {
                    ...
                    if let Err(err) = conn.tx_to_local_stream.send(msg.to_owned()) {
                        conn.on_close(&err.to_string(), false).await;
                        break;
                    }
                },
    ...
    async fn handle_processing_request_without_validation(
        &mut self,
        processing_request: &ProcessingRequest,
    ) {
        self.processing_request = processing_request.clone();
        if let Some(o) = processing_request.option.as_ref() {
        ...
        self.video_ack_required = processing_request.video_ack_required;
    }
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        if let Some(message::Union::ProcessingRequest(processing_request)) = msg.union {
            self.handle_processing_request_without_validation(&processing_request)
                .await;
            if self.processing {
                return true;
            }
            match processing_request.union {
                Some(processing_request::Union::FileTransfer(ft)) => {
                    log::info!("File transfer request: {:#?}", ft);
                }
                Some(processing_request::Union::PortForward(mut pf)) => {
                    log::info!("File transfer request: {:#?}", pf);
                }
                _ => {}
            }
            if !hbb_common::is_ipv4_str(&processing_request.username)
                && processing_request.username != Config::get_id()
            {
                self.send_processing_error("Offline").await;
            } else if password::approve_mode() == ApproveMode::Click
                || password::approve_mode() == ApproveMode::Both && !password::has_valid_password()
            {
                self.try_start_cm(processing_request.my_id, processing_request.my_name, false);
                if hbb_common::get_version_number(&processing_request.version)
                    >= hbb_common::get_version_number("1.2.0")
                {
                    self.send_processing_error("No Password Access").await;
                }
                return true;
            } else if password::approve_mode() == ApproveMode::Password
                && !password::has_valid_password()
            {
                self.send_processing_error("Connection not allowed").await;
                return false;
            } else if processing_request.password.is_empty() {
                self.try_start_cm(processing_request.my_id, processing_request.my_name, false);
            } else {
            ...
            match msg.union {
                ...
                Some(message::Union::FileAction(fa)) => match fa.union {
                    _ => {
                        log::info!("FileAction");
                    }
                },
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::CloseReason(_)) => {
                        ...
                        SESSIONS
                            .lock()
                            .unwrap()
                            .remove(&self.processing_request.my_id);
                        return false;
                    }
    ...
    async fn send_processing_error<T: std::string::ToString>(&mut self, err: T) {
        let mut msg_out = Message::new();
        let mut res = LoginResponse::new();
    ...
    fn send(&mut self, msg: Message) {
        log::info!("Send {}", msg);
        allow_err!(self.tx_to_local_stream.send(msg));
    }
...
async fn start_ipc_stream(
    ...
    let mut stream = None;
    log::info!("start_ipc: {}", postfix);
    if let Ok(s) = crate::ipc::connect(1000, &format!("_{}", postfix)).await {
        stream = Some(s);
    } else {
        let arg = format!("--{}", postfix);
        ...
        if stream.is_none() {
            bail!("Failed IPC to connect to {}", postfix);
        }
    }
